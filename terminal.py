...
class WSLTerminalMod(loader.Module):
    ...
    async def _run_wsl(self, distro, command):
        wrapped = "wsl.exe -d {} -- bash -lc {}".format(shlex.quote(distro), shlex.quote(command))
    ...
    def _format(self, target, command, code, output):
        ...
        return "💻 <b>{}</b> <code>{}</code>\n<code>{}</code>\n\n<pre>{}</pre>".format(html.escape(target), html.escape(status), html.escape(command), html.escape(output))
    ...
    async def t(self, message):
        ...
        if first in {"ubuntu", "debian", "win", "windows"}:
            target = first
        ...
        if target in {"win", "windows"}:
            code, output = await self._run_shell("cmd.exe /d /s /c {}".format(shlex.quote(command)))
            display = "Windows"
        else:
